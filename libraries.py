...
def resample_and_merge2(df1_n, df2_n, freq='1S', time_column_df1='Dataloggertijd, in s',
                       time_column_df2='Dataloggertijd, in s'):
    ...
    df1_resampled = df1.resample(freq).mean()  # .mean() for all numeric columns
    df2_resampled = df2.resample(freq).mean()

    # Optionally, select only numeric columns if your data contains non-numeric columns
    df1_resampled = df1_resampled.select_dtypes(include=[np.number])
    df2_resampled = df2_resampled.select_dtypes(include=[np.number])
